    ...
    private List<DataTracker.SerializedEntry<?>> polymer$createEntries(List<DataTracker.SerializedEntry<?>> trackedValues) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (entry.value() instanceof VillagerData data) {
                ...
                if (x != null) {
                    entries.set(i, new DataTracker.SerializedEntry(entry.id(), entry.handler(), data.withProfession(Registries.VILLAGER_PROFESSION.getEntry(x.getPolymerReplacement(data.getProfession(), player)).value())));
                }
            }
        }
